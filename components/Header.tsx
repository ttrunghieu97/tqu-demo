...
const Header: React.FC<NavigationProps> = ({ department }) => {
	...
	// Function to render menu items, wrapped in useCallback for performance
	const renderMenuItem = useCallback((item: MenuItem) => {
		...
		return (
			<li className="relative group">
				<button
					aria-haspopup="true"
					className="text-gray-700 dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400 px-3 py-2 rounded-md text-lg font-bold uppercase inline-flex items-center"
				>
					{item.label}
				</button>
				<div
					className={`absolute left-1/2 transform -translate-x-1/2 mt-2 ${getDropdownWidthClass(item.columns)} rounded-md shadow-lg bg-white dark:bg-gray-700 ring-1 ring-black ring-opacity-5 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300`}
				>
					<div className={`py-1 grid ${getGridColumnsClass(item.columns)} gap-4`} role="menu" aria-orientation="vertical">
						{item.items?.map((column, idx) => (
							<div key={idx}>
								{column.title && <div className="font-bold text-center">{column.title}</div>}
	...
	return (
		<>
			...
			<nav className={`bg-white dark:bg-black shadow-lg ${isSticky ? 'fixed top-0 left-0 w-full z-50' : ''}`}>
