  ...
  const fetchPosts = useCallback(async (page: number) => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}items/noi_quy_quy_che?limit=${postsPerPage}&page=${page}&sort=-created_at&meta=total_count`
      );
    ...
    <div className="px-4 mt-5 dark:bg-gray-900">
      <div className="container mx-auto mb-5">
        <div className="text-center mb-5">
          <h1 className="text-4xl font-bold text-gray-900 dark:text-white mb-4">Nội quy - Quy chế</h1>
        </div>
        <div>{renderPosts}</div>
        {renderPagination}
      </div>
