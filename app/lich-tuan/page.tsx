...
export default function SinhVienTieuBieu() {
  ...
  const fetchPosts = useCallback(async (page: number) => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}items/lich_tuan?limit=${postsPerPage}&page=${page}&sort=-created_at&meta=total_count`
      );
  ...
  const renderPosts = useMemo(
    () =>
      posts.length > 0 ? (
        posts.map((post) => (
          <Link href={`/lich-tuan/${post.slug}`} key={post.id}>
