...
export default function TuyenDung() {
  ...
  const fetchPosts = useCallback(async (page: number) => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}items/tuyen_dung?limit=${postsPerPage}&page=${page}&sort=-created_at&meta=total_count`
      );
  ...
  const renderPosts = useMemo(
    () =>
      posts.length > 0 ? (
        posts.map((post) => (
          <Link href={`/tuyen-dung/${post.slug}`} key={post.id} className="block h-full">
